...
import footerSection from "../data/footerSection.json";
...
function FooterPage() {
  ...
  return (
    <>
      <nav className="mb-0 mt-4 mx-auto w-full px-[3%] pb-2">
        <div className="p-0 m-0 border-0 align-baseline before:content-[''] before:bg-white before:border-b before:border-b-[rgb(266,266,266)]  before:border-t-2 before:border-t-[rgb(226,226,226)]  before:block before:h-[1px] before:mt-0 before:align-baseline">
          <header className="flex flex-row">
            <a aria-label="New York Times" href="/">
              <svg
                className="block h-11 align-middle w-[184px]"
                viewBox="0 0 184 25"
                fill="#000"
                aria-hidden="true"
              >
          ...
          {!isMobile ? (
            <div className="block">
              <div className="flex mt-3 min-w-[600px]">
                {Object.keys(sections).map((section) => (
                  <section key={section} className="flex-1 block">
                    <h3 className="text-[rgb(51,51,51)] text-[13px] font-[franklin-700] h-6 leading-[15px] m-0 uppercase w-36">
                      {section}
                    </h3>
                    ...
                    <ul className="m-0 p-0">
                      {sections[section].map((item) => (
                        <li
                          className="m-0 p-0 border-0 align-baseline list-item"
                          key={item.params}
                        >
                          <Link
                            to={`/section/${item.params}`}
                            className="text-[rgb(0,0,0)] inline-block font-[franklin-500] w-36 text-[14px] leading-4 h-6"
                            onClick={() => handleSubsectionChange(item.params)}
                          >
                            {item.name}
                          </Link>
                        </li>
                ...
                <div className="border-l border-l-[rgb(226,226,226)] flex-1 pl-4">
                  <h3 className="text-[rgb(51,51,51)] text-sm font-[franklin-700] h-6 m-0 uppercase w-[150px]">
                    Account
                  </h3>
                  <ul className="list-none m-0 p-0">
                    <li className="mb-1 whitespace-nowrap">
                      ...
                      >
                        <svg
                          className="inline-block h-3 w-3 mr-2 align-middle"
                          viewBox="0 0 10 13"
                        >
                          <path
                            fill="#000"
            ...
            <div>
              {Object.keys(sections).map((section) => (
                <div
                  key={section}
                  className="border-t border-t-[rgb(235,235,235)]"
                >
                  <button
                    onClick={() => handleSectionClick(section)}
                    className={`m-0 font-[franklin-700] text-[13px] h-11 leading-[44px] uppercase ${
                      openSection === section
                  ...
                  <div>
                    {openSection === section && (
                      <ul className="m-0 pb-4 grid grid-cols-2">
                        {sections[section].map((item, index) => (
                          <li className="h-9 leading-8" key={index}>
                            <Link
                              to={`/section/${item.params}`}
                              className="text-[rgb(51,51,51)] block font-[franklin-500] text-[15px] h-8 leading-8 capitalize"
                              onClick={() =>
                                handleSubsectionChange(item.params)
                              }
                            >
                              {item.name}
                            </Link>
      ...
      <footer className="my-0 mx-auto px-[3%] pb-2 pt-0 text-[11px] block text-center">
        <nav className="border-t pt-2 mb-9 block">
          <ul className=" mb-2 p-0 inline-block">
            <li>
              ...
              <span>
                , created for educational purposes. None of the content
                presented here is affiliated with or authorized by The New York
                Times.
              </span>
            </li>
          </ul>
          <ul className=" leading-[8px] inline-block p-0 ">
            <li className="border-l-0 inline-block leading-5 py-0 px-10">
              <span>&#169; 2025 </span>
              <a
              ...
              <span>
                . Educational project inspired by the design of the New York
                Times. No official affiliation.
              </span>
            </li>
            <li className="border-l-0 inline-block leading-5 py-0 px-10">
              <span>
                Project developed using React, Redux, Axios and Tailwind CSS.
